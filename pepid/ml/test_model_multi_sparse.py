...
from train_model_multi_sparse import MsDataset, import_all

imp = __import__("spectrum_generator_multi_sparse", fromlist=["Model", "embed", "PROT_BLIT_LEN", "PROT_STR_LEN", "MAX_CHARGE"])
Model = getattr(imp, "Model")

import_all("spectrum_generator_multi_sparse")
...
model.load_state_dict(torch.load("ml/{}.pkl".format("spectrum_generator_multi_sparse")))
...
def show_one():
    ...
    for batch in dl_test:
        ...
        pred_select = pred_select * (pred_select >= 1e-5)

        print("{}\n".format(((pred_select / torch.norm(pred_select, p=2, dim=-1, keepdim=True)) * (spec / torch.norm(spec, p=2, dim=-1, keepdim=True))).sum(dim=-1).mean().detach()))
